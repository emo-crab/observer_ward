...
pub fn cluster_templates(templates_list: &[Template]) -> ClusterType {
  ...
  for mut template in templates_list.iter().cloned() {
    ...
    match template.compile() {
      Ok(_) => {
        ...
        {
          compile_templates_list.push(Arc::new(template));
        }
        if let Some(fav) = favicon {
          favicon_cops.push(ClusteredOperator::new(Arc::new(fav)));
        }
      }
      Err(err) => {
  ...
  if !favicon_cops.is_empty() {
    executes.web_favicon.push(Arc::new(ClusterExecute {
  ...
  if executes.web_default.is_empty() && !executes.web_favicon.is_empty() {
    executes.web_default.push(Arc::new(ClusterExecute {
      requests: Arc::new(Requests::default_web_index()),
      rarity: 0,
      operators: vec![],
    }))
  }
...
// 分类优化请求
fn cluster(list: &[Arc<Template>]) -> Vec<Vec<Arc<Template>>> {

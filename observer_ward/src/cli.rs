...
pub struct ObserverWardConfig {
  ...
  /// poc afrog engine additional args
  #[argh(option)]
  #[serde(skip)]
  pub fargs: Option<String>,
  /// poc engine "nuclei", "afrog" or "all", default nuclei
  #[argh(option)]
  #[serde(skip)]
...
impl ObserverWardConfig {
  ...
  pub fn enable_engine(&self) -> bool {
    self.path.is_some() || self.plugins.is_some() || self.engine.is_some()
  }
}
